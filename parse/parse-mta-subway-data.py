...
Run download-mta-subway-gtfs.py first to get the GTFS data.
"""
...
class MTASubwayDataParser:
    def __init__(self):
        ...
        if not self.gtfs_dir.exists():
            raise FileNotFoundError(
                ...
                f"Please run 'python download-mta-subway-gtfs.py' first to download the data."
            )
    ...
    def run(self):
        ...
        self.save_json(route_data, 'mta-subway-routes-data.json')
        
        # Save JS file with same structure (includes tripToRoute and tripToHeadsign)
        self.save_js(route_data, 'mta-subway-routes-data.js', 'mtaSubwayRoutesData')
...
def main():
    try:
        ...
        print("   python download-mta-subway-gtfs.py")
        return 1
    except Exception as e:
        print(f"\n❌ Error: {e}")
        import traceback
