...
Run download-lirr-gtfs.py first to get the GTFS data.
"""
...
class LIRRDataParser:
    def __init__(self):
        ...
        if not self.gtfs_dir.exists():
            raise FileNotFoundError(
                ...
                f"Please run 'python download-lirr-gtfs.py' first to download the data."
            )
    ...
    def run(self):
        ...
        self.save_json(route_data, 'lirr-routes-data.json')
        
        # Save JS file with same structure (includes tripToRoute and tripToHeadsign)
        self.save_js(route_data, 'lirr-routes-data.js', 'lirrRoutesData')
...
def main():
    try:
        ...
        print("   python download-lirr-gtfs.py")
        return 1
    except Exception as e:
        print(f"\n❌ Error: {e}")
        return 1
