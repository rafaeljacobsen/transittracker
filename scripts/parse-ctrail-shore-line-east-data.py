#!/usr/bin/env python3
"""
CTrail Shore Line East Data Parser - Generates route and station JSON files from local GTFS
Parses Shore Line East GTFS data from the shore_line_east_gtfs/ directory (downloaded separately)
...
class ShoreLineEastDataParser:
    ...
    def parse_stops(self):
        ...
        print("\n🏢 Parsing Shore Line East stations...")
        ...
        print(f"✅ Found {len(stations)} Shore Line East stops/stations")
        return stations
    ...
    def save_js(self, data, filename, var_name):
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"// Shore Line East Data - Generated from GTFS\n")
                f.write(f"{var_name} = ")
                json.dump(data, f, indent=2, ensure_ascii=False)
                f.write(";\n\n")
    ...
    def run(self):
        ...
        print("=" * 60)
        print("🚂 SHORE LINE EAST DATA PARSER")
        print("=" * 60)
        ...
        self.save_json(route_data, 'data/shore-line-east-routes-data.json')
        
        # Save JS file
        self.save_js(route_data, 'data/shore-line-east-routes-data.js', 'shoreLineEastRoutesData')
        
        print("\n" + "=" * 60)
        print("✅ SHORE LINE EAST DATA PARSING COMPLETE!")
        print("=" * 60)
        print("\nGenerated files:")
        print("  📄 shore-line-east-routes-data.json (reference)")
        print("  📄 shore-line-east-routes-data.js (for website)")
        print(f"\n✨ Processed {route_data['totalRoutes']} Shore Line East routes")
...
def main():
    try:
        parser = ShoreLineEastDataParser()
        parser.run()
    except FileNotFoundError as e:
        print(str(e))
        print("\n💡 To download Shore Line East GTFS data, run:")
