#!/usr/bin/env python3
"""
CTrail Hartford Line Data Parser - Generates route and station JSON files from local GTFS
Parses Hartford Line GTFS data from the hartford_line_gtfs/ directory (downloaded separately)

Run scripts/download-ctrail-hartford-line-gtfs.py first to get the GTFS data.
"""
...
class HartfordLineDataParser:
    def __init__(self):
        self.gtfs_dir = Path("hartford_line_gtfs")
        
        # Check if GTFS directory exists
        if not self.gtfs_dir.exists():
            raise FileNotFoundError(
                f"\n❌ GTFS directory not found: {self.gtfs_dir}\n"
                f"Please run 'python scripts/download-ctrail-hartford-line-gtfs.py' first to download the data."
            )
    ...
    def parse_routes(self):
        """Parse routes.txt to get Hartford Line route information"""
        print("\n🚂 Parsing Hartford Line routes...")
        ...
        print(f"✅ Found {len(routes)} Hartford Line routes")
    ...
    def parse_stops(self):
        ...
        print("\n🏢 Parsing Hartford Line stations...")
        ...
        print(f"✅ Found {len(stations)} Hartford Line stops/stations")
        return stations
    ...
    def generate_route_data(self):
        ...
        route_data = {
            ...
            'source': 'CTrail Hartford Line GTFS',
            'agency': 'Hartford Line',
    ...
    def save_js(self, data, filename, var_name):
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"// Hartford Line Data - Generated from GTFS\n")
                f.write(f"{var_name} = ")
                json.dump(data, f, indent=2, ensure_ascii=False)
                f.write(";\n\n")
    ...
    def run(self):
        ...
        print("=" * 60)
        print("🚂 HARTFORD LINE DATA PARSER")
        print("=" * 60)
        ...
        self.save_json(route_data, 'data/hartford-line-routes-data.json')
        
        # Save JS file
        self.save_js(route_data, 'data/hartford-line-routes-data.js', 'hartfordLineRoutesData')
        
        print("\n" + "=" * 60)
        print("✅ HARTFORD LINE DATA PARSING COMPLETE!")
        print("=" * 60)
        print("\nGenerated files:")
        print("  📄 hartford-line-routes-data.json (reference)")
        print("  📄 hartford-line-routes-data.js (for website)")
        print(f"\n✨ Processed {route_data['totalRoutes']} Hartford Line routes")
...
def main():
    try:
        parser = HartfordLineDataParser()
        parser.run()
    except FileNotFoundError as e:
        print(str(e))
        print("\n💡 To download Hartford Line GTFS data, run:")
        print("   python scripts/download-ctrail-hartford-line-gtfs.py")
        return 1
    except Exception as e:
        print(f"\n❌ Error: {e}")
        return 1
