...
Run scripts/download-metro-north-gtfs.py first to get the GTFS data.
"""
...
class MetroNorthDataParser:
    def __init__(self):
        ...
        if not self.gtfs_dir.exists():
            raise FileNotFoundError(
                ...
                f"Please run 'python scripts/download-metro-north-gtfs.py' first to download the data."
            )
    ...
    def run(self):
        ...
        self.save_json(route_data, 'data/metro-north-routes-data.json')
        
        # Save JS file with same structure (includes tripToRoute and tripToHeadsign)
        self.save_js(route_data, 'data/metro-north-routes-data.js', 'metroNorthRoutesData')
...
def main():
    try:
        ...
        print("   python scripts/download-metro-north-gtfs.py")
        return 1
    except Exception as e:
        print(f"\n❌ Error: {e}")
        return 1
