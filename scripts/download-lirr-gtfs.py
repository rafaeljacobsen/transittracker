#!/usr/bin/env python3
"""
Download MTA LIRR GTFS Data
Downloads the latest LIRR GTFS feed and extracts it to the lirr_gtfs/ directory
"""
...
# MTA LIRR GTFS Feed URL
LIRR_GTFS_URL = "http://web.mta.info/developers/data/lirr/google_transit.zip"

def download_lirr_gtfs():
    """Download and extract LIRR GTFS data"""
    print("=" * 60)
    print("MTA LIRR GTFS DOWNLOADER")
    print("=" * 60)
    print(f"Downloading from: {LIRR_GTFS_URL}")
    print()
    
    try:
        # Download the GTFS zip file
        print("📥 Downloading LIRR GTFS data...")
        response = requests.get(LIRR_GTFS_URL, timeout=120)
        ...
        gtfs_dir = Path("lirr_gtfs")
        if gtfs_dir.exists():
            backup_dir = Path(f"lirr_gtfs_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        print()
        print("✨ Ready to parse! Run scripts/parse-lirr-data.py to generate data.")
...
def main():
    """Main function"""
    success = download_lirr_gtfs()
    
    if success:
        print("\n🎉 Success! You can now run:")
        print("   - python scripts/parse-lirr-data.py")
        print()
        print("💡 Note: LIRR data is ready for your transit tracker!")
    else:
